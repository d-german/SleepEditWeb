...
| `SleepEditWeb/Infrastructure/SleepNote/LiteDbSleepNoteConfigRepository.cs` | LiteDB config persistence (shared sleepeditweb.db) |
...
3. **Shared LiteDB instance**: All repos share a single `sleepeditweb.db` via DI singleton — config persists across all instances
