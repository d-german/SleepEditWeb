...
Require the hard-coded password `sleep123` before any Admin page or operation can be used. A successful login unlocks Admin for the existing 30-minute idle session. Logout revokes access immediately.
...
An Admin password middleware runs after `UseSession` and before route execution. It checks requests whose path begins with `/Admin`, while allowing the login endpoints and static resources to proceed.
...
The middleware protects current and future Admin controller routes without relying on each action author to remember a check.
...
- Export and all mutation endpoints are protected by the same session gate.
...
- Middleware tests proving anonymous Admin requests are blocked, login remains reachable, unlocked sessions pass, and protected non-GET requests cannot reach their action.
